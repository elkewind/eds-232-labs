...
```{r}
sales_plot <- ggplot(pumpkins, aes(x = day, y = price)) +
  geom_point()
sales_plot
```
...
**This does not seem like a very useful plot to me. Prices in September, October, and November vary greatly. It is unsurprising to me that August and December have less variation because less people are probably buying pumpkins then. What is a bit surprising is the gap around 25 in both October and November.**
...
pumpkins_by_month <- new_pumpkins %>% 
  group_by(month) %>% 
...
sales_by_month_plot <- pumpkins_by_month %>% 
...
**Variety is most highly correlated with price. This could be because certain types of pumpkins are more rare or take more resources to grow and therefore more expensive in the market.**
...
# Create a recipe for preprocessing the data
      ...
      # Question here -- what do the following lines do?
...
**The categorical package types are ordered arbitrarily, resulting in the highest prices in the "2" package rather than the "4" package. The data do not look linear.**
