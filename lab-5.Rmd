...
forest_spec <- 
  ...
  mtry = tune(),
  trees =  150) %>% # 15 parameters * 10
  set_engine("ranger") %>%
  set_mode("classification")

forest_grid <- grid_regular(min_n(), mtry(c(1,13)), levels = 5)
...
print(paste0("We see here that our random forest had the highest accuracy at predicting listener than the. The accuracy of the forest was ", round(final_forest_accuracy, 3), "."))
```
